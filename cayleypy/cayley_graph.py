...
class CayleyGraph:
    ...
    def __init__(
        ...
        **unused_kwargs,
        ...
        :param num_gpus: Number of GPUs for distributing seen_states_hashes in BFS.
    ...
    def _bfs_layer_distributed(
        ...
        # Each GPU processes batch_size/ng states per batch round.
        ...
        per_gpu_batches = [
            list(layer1_parts[g].tensor_split(num_batches, dim=0)) for g in range(ng)
        ]

        for b in range(num_batches):
            # Phase 1: Parallel neighbor gen + local dedup on each GPU.
            phase1_results = [None] * ng  # type: list[Optional[torch.Tensor]]
            for g in range(ng):
                chunk = per_gpu_batches[g][b]
                dev = self.gpu_devices[g]
                if len(chunk) == 0:
                    phase1_results[g] = torch.empty(0, dtype=torch.int64, device=dev)
                    continue
                stream = streams[g]
            ...
            # Each GPU g partitions its unique hashes by ownership and sends to target GPU.
            send_bufs = [[None] * ng for _ in range(ng)]  # type: list[list[Optional[torch.Tensor]]]
            for g in range(ng):
                hashes_g = phase1_results[g]
                if len(hashes_g) == 0:
                    for t in range(ng):
                        send_bufs[g][t] = torch.empty(0, dtype=torch.int64, device=self.gpu_devices[t])
                    continue
            ...
            for g in range(ng):
                ...
                with torch.cuda.stream(stream):
                    ...
                    # Free send buffers for this target immediately.
                    for src in range(ng):
                        send_bufs[src][g] = None
                    if len(received) == 0:
                        continue
    ...
    def bfs(
        ...
        *,
        ...
        :param cpu_offload: Level of CPU offloading for hash filtering (only useful when device is cuda).
               0 (default): all computation on self.device.
               1: seen_states_hashes stored on CPU; filtering (isin_via_searchsorted) runs on CPU.
               2: additionally, batch result accumulation on CPU; final hstack+sort on CPU.
        ...
        # CPU offloading: when enabled, seen hashes (and optionally batch results) live on CPU.
        filter_device = torch.device("cpu") if cpu_offload >= 1 and self.device.type == "cuda" else self.device
        ...
        use_distributed = self.num_gpus > 1 and cpu_offload == 0 and do_batching
        if use_distributed:
            ng = self.num_gpus
        ...
        if not use_distributed:
            seen_states_hashes = [layer1_hashes.to(filter_device)]
        ...
        def _remove_seen_states(current_layer_hashes: torch.Tensor) -> torch.Tensor:
            ...
            if filter_device != self.device:
                h_cpu = current_layer_hashes.to(filter_device)
                ans = ~isin_via_searchsorted(h_cpu, seen_states_hashes[-1])
                for h in seen_states_hashes[:-1]:
                    ans &= ~isin_via_searchsorted(h_cpu, h)
        ...
        def _end_of_iteration_non_distributed(layer2_, layer2_hashes_):
            ...
            if not use_distributed:
                seen_states_hashes.append(layer2_hashes_.to(filter_device))
                if self.definition.generators_inverse_closed:
        ...
        for i in range(1, max_diameter + 1):
            if use_distributed and sum(len(p) for p in layer1_parts) > self.batch_size:
                ...
                accepted_parts = [
                    torch.empty(0, dtype=torch.int64, device=self.gpu_devices[g]) for g in range(ng)
                ]
                accepted_parts = self._bfs_layer_distributed(
                    layer1_parts, seen_parts, accepted_parts, _dist_streams,
                )
                ...
                if return_all_hashes:
                    all_layers_hashes.append(
                        torch.cat([p.to(self.device) for p in layer1_parts]).sort()[0]
                    )
                if layer2_size == 0:
                ...
                # Update seen_parts: append new layer's hashes to per-GPU lists.
                if self.definition.generators_inverse_closed:
                    # Only keep last 2 layers.
                    for g in range(ng):
                        seen_parts[g] = []
                ...
                for layer1_batch in layer1.tensor_split(num_batches, dim=0):
                    ...
                    if cpu_offload >= 2 and layer2_hashes_batches:
                        h_cpu = layer2_hashes_batch.to(filter_device)
                        for other_batch_hashes in layer2_hashes_batches:
                            mask &= ~isin_via_searchsorted(h_cpu, other_batch_hashes).to(self.device)
                    else:
                        for other_batch_hashes in layer2_hashes_batches:
                    ...
                    if cpu_offload >= 2:
                        layer2_batches.append(layer2_batch.to("cpu"))
                        layer2_hashes_batches.append(layer2_hashes_batch.to("cpu"))
                    else:
                ...
                if cpu_offload >= 2:
                    ...
                    layer2_hashes = layer2_hashes.to(self.device)
