...
class BfsDistributed:
    ...
    def _bfs_layer_distributed(
        ...
        for batch_id in range(num_batches):
            ...
            send_states = [[cls._empty_part(device, graph.encoded_state_size)[0] for device in graph.gpu_devices] for _ in range(graph.num_gpus)]
            send_hashes = [[torch.empty(0, dtype=torch.int64, device=device) for device in graph.gpu_devices] for _ in range(graph.num_gpus)]
